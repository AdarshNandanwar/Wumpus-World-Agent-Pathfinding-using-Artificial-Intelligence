...
def dpll(clauses, symbols, model):
    ...
    total_dpll_calls += 1
    if total_dpll_calls > max_dpll_calls:
        return None
...
def hybrid_wumpus_agent(ag, kb, symbols, model):
    ...
    # Manhattan Distance Room Ordering HEURISTIC
        ...
        for room in rooms:
            if room not in safe:
                ...
                while res == None:
                    ...
                    if DEBUG: print("dpll {0}: max calls {1}, dpll calls {2}".format('r'+room[1:], max_dpll_calls, total_dpll_calls))
                is_safe = (res == False)
