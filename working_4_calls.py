        ...
        if next_room == None:
            if len(safe) == len(visited):
                ...
                for unvisited_room in unvisited:
                    ...
                    # print("CHECKING IF NO PIT AT [{0}, {1}]".format(x,y))
                    kb_p = kb.copy()
                    kb_p.add('p'+unvisited_room[1:])
                    res = dpll(kb_p, symbols, {})
                    is_no_pit = (res == False)
                    if is_no_pit:
                        kb.add('!p'+unvisited_room[1:])

                    # print("CHECKING IF NO WUMPUS AT [{0}, {1}]".format(x,y))
                    kb_w = kb.copy()
                    kb_w.add('w'+unvisited_room[1:])
                    res = dpll(kb_w, symbols, {})
                    is_no_wumpus = (res == False)
                    if is_no_wumpus:
                        kb.add('!w'+unvisited_room[1:])

                    if DEBUG: print("dpll {0}: no pit {1}, no wumpus {2}".format('r'+unvisited_room[1:], is_no_pit, is_no_wumpus))
                    if is_no_pit and is_no_wumpus:
                    ...
                    # print("CHECKING IF PIT AT [{0}, {1}]".format(x,y))
                    kb_p = kb.copy()
                    kb_p.add('!p'+unvisited_room[1:])
                    res = dpll(kb_p, symbols, {})
                    is_pit = (res == False)
                    ...
                    # print("CHECKING IF WUMPUS AT [{0}, {1}]".format(x,y))
                    kb_w = kb.copy()
                    kb_w.add('!w'+unvisited_room[1:])
                    res = dpll(kb_w, symbols, {})
                    is_wumpus = (res == False)
                    if is_wumpus:
                        kb.add('w'+unvisited_room[1:])
                    if DEBUG: print("dpll {0}: pit {1}, wumpus {2}".format('r'+unvisited_room[1:], is_pit, is_wumpus))
